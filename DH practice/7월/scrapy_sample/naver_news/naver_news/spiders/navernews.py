...
url_format = "https://search.naver.com/search.naver?date_from={0}&date_option=8&date_to={0}&dup_remove=1&nso=so%3Add%2Cp%3Afrom{0}to{0}&query={1}&sm=nws_hty&srchby=all&st=date&where=news&start={2}"
...
class NaverNewsSpider(scrapy.Spider):
    ...
    def parse(self, response):   
        item = NaverNewsItem()
        for a_tag in response.css('li'):
            item['title'] = a_tag.css('dl dt a::attr(title)').get()
            item['url'] = a_tag.css('dl dt a::attr(href)').get()
            yield item
    ...
    #     query_str = parse.parse_qs(parse.urlsplit(response.url).query)
    #     item['author'] = query_str['newsid'][0]
